...
const MyJobs = () => {
  ...
  const handleDeleteJob = async (id) => {
    ...
    try {
      ...
      if (res.ok && data.acknowledged) {
        ...
        alert(data.message || "You cannot delete this job.");
      }
    } catch (error) {
      console.error(error);
      ...
      if (data.acknowledged) {
        if (isAdmin) {
          setJobApplicants((prev) =>
            prev.filter((a) => String(a._id) !== String(applicationId)),
          );
        } else {
  ...
  const handleViewApplicants = async (job) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/applications/${job._id}?email=${encodeURIComponent(user.email)}`,
      );
      const data = await res.json();
  ...
  const handleUpdateStatus = async (appId, updates) => {
    try {
      ...
      if (data.acknowledged || data.modifiedCount > 0) {
        setJobApplicants((prev) =>
          prev.map((a) =>
            String(a._id) === String(appId) ? { ...a, ...updates } : a,
          ),
        );
      }
    } catch (err) {
  ...
  const handleToggleJobStatus = async (job) => {
    ...
    try {
      ...
      if (data.acknowledged || data.modifiedCount > 0) {
        setJobs((prev) =>
          prev.map((j) =>
            String(j._id) === String(job._id) ? { ...j, status: newStatus } : j,
          ),
        );
      }
    } catch {
  ...
  const safeItems =
    isAdmin || userProfile?.role === "employer"
      ? jobs
      : activeTab === "saved"
        ? savedJobs
        : applications;
  ...
  return (
    <div className="min-h-screen relative" style={{ background: "#f8fafc" }}>
      ...
      <div
        style={{
          background: "linear-gradient(135deg, #1e3a8a 0%, #3575E2 100%)",
        }}
        ...
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="text-white">
            ...
            <p className="text-blue-200 text-sm">
              {isAdmin || userProfile?.role === "employer"
                ? "Manage all jobs you have posted and review applicants"
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20">
            ...
            {(isAdmin || userProfile?.role === "employer") && (
              <Link
                to="/post-job"
                className="mt-4 inline-flex items-center gap-2 text-blue-600 hover:underline font-medium text-sm"
              >
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-50">
                  {currentItems.map((job, index) => (
                    <tr
                      key={job._id || index}
                      className={`hover:bg-gray-50 transition-colors ${job.status === "closed" ? "opacity-70" : ""}`}
                    >
                      ...
                      <td className="px-6 py-4">
                        {job.status === "closed" ? (
                          <span className="text-xs bg-red-50 text-red-700 border border-red-100 px-2 py-1 rounded-full font-semibold">
                            Closed
                          </span>
                        ) : (
                          <span className="text-xs bg-green-50 text-green-700 border border-green-100 px-2 py-1 rounded-full font-semibold">
                            Active
                          </span>
                        )}
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex flex-col gap-1.5 w-max">
                          <button
                            ...
                            className={`text-[10px] px-2 py-1 rounded border text-left transition-colors ${job.featured ? "bg-yellow-50 border-yellow-200 text-yellow-700 font-bold" : "bg-gray-50 border-gray-200 text-gray-500 hover:bg-gray-100"}`}
                          >
                            ⭐ Featured
                          </button>
                          <button
                            onClick={() =>
                              handleToggleBadge(job._id, "urgent", job.urgent)
                            }
                            className={`text-[10px] px-2 py-1 rounded border text-left transition-colors ${job.urgent ? "bg-orange-50 border-orange-200 text-orange-700 font-bold" : "bg-gray-50 border-gray-200 text-gray-500 hover:bg-gray-100"}`}
                          >
                            ⚡ Urgent
                          </button>
                          <button
                            ...
                            className={`text-[10px] px-2 py-1 rounded border text-left transition-colors ${job.verified ? "bg-blue-50 border-blue-200 text-blue-700 font-bold" : "bg-gray-50 border-gray-200 text-gray-500 hover:bg-gray-100"}`}
                          >
                            ✅ Verified
                          </button>
                        </div>
                  ...
                  >
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <button
                        onClick={async () => {
                          await fetch(
                            `${API_BASE_URL}/saved-jobs/${item._id}?email=${encodeURIComponent(user.email)}`,
                            { method: "DELETE" },
                          );
                          setSavedJobs((prev) =>
                            prev.filter(
                              (j) => String(j._id) !== String(item._id),
                            ),
                          );
                        }}
                    ...
                    <div className="border-t border-gray-50 pt-3 flex items-center justify-between">
                      <span className="text-xs text-gray-400">
                        Posted: {formatDate(item.postingDate)}
                      </span>
                      <Link
                        to={`/job/${item._id}`}
              ...
              return (
                ...
                >
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <span
                      className={`text-xs border px-2.5 py-1 rounded-full font-medium flex-shrink-0 ml-3 capitalize ${statusClass}`}
                    >
                      {currentStatus}
                    </span>
                  </div>
                  ...
                  <div className="border-t border-gray-50 pt-3 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-4">
                      <button
                        onClick={() => handleWithdrawOrReject(app._id, false)}
                        className="text-xs font-medium text-red-500 hover:text-red-700 transition-colors"
                      >
                        Withdraw
                      </button>
                      <a
                        href={`${API_BASE_URL}${app.resumeLink}`}
                        target="_blank"
            ...
            <div className="flex items-center justify-between p-6 border-b border-gray-100">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">
                  {selectedJob?.jobTitle} • {jobApplicants.length} Total
                </p>
              </div>
              <button
              ...
              {isLoadingApplicants ? (
                ...
                <div className="space-y-4">
                  {jobApplicants.map((applicant) => (
                    <div
                      key={applicant._id}
                      className="bg-gray-50 rounded-xl p-4 sm:p-5 border border-gray-100"
                    >
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                        ...
                        <div className="flex flex-wrap items-center gap-2 sm:flex-nowrap sm:flex-shrink-0">
                          ...
                          <button
                            onClick={() =>
                              handleWithdrawOrReject(applicant._id, true)
                            }
                      ...
                      <div className="pt-3 mt-3 border-t border-gray-100">
                        ...
                        <p className="text-[10px] text-gray-400 mt-1">
                          Notes are auto-saved on click away. Only visible to
                          you.
                        </p>
                      </div>
                    </div>
